...
exports.SmartBridgeAction = exports.SmartBridgeInput = void 0;
...
`;
exports.SmartBridgeInput = zod_1.z
    .object({
    fromChainId: zod_1.z
        .number()
        .int()
    ...
    recipientAddress: zod_1.z
        ...
        .optional()
        .nullable()
        .describe("Optional: The address to receive tokens on the destination chain. Defaults to your wallet address."),
    slippage: zod_1.z
        .string()
        .optional()
        .nullable()
        .default("1") // Defaulting to 1%
        .describe("Optional: Slippage tolerance in percentage (e.g., '0.5' for 0.5%). Default is '1'."),
    approveMax: zod_1.z
        .boolean()
        .optional()
        .nullable()
        .default(false)
        .describe("Optional: Whether to approve maximum token allowance for the input token. Default is false."),
    payProtocolFee: zod_1.z
        .boolean()
        .optional()
        .nullable()
        .default(true) // CHANGED: Default to true as deBridge requires this.
        .describe("Optional: Whether to include the deBridge protocol fee. Default is true. Smart account needs NATIVE currency for this."),
})
    .strip()
    .describe("Instructions for bridging tokens cross-chain via Debridge DLN");
async function smartBridge(wallet, args) {
    ...
    try {
        ...
        let formattedAmount;
        try {
            formattedAmount = await (0, services_1.formatTokenAmount)(wallet, args.tokenInAddress, args.amount);
        }
        catch (error) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorDetail = apiResponseData.error ||
        ...
        if (tx.allowanceTarget && tx.allowanceValue) {
            ...
            try {
                const approvalResult = await (0, services_1.checkAndApproveTokenAllowance)(wallet, args.tokenInAddress, tx.allowanceTarget, BigInt(tx.allowanceValue), // Amount to approve from API
                true);
                ...
                console.log(`Preliminary token approval successful for ${args.tokenInAddress} to ${tx.allowanceTarget}. UserOpHash: ${approvalResult.userOpHash}`);
            }
            catch (error) {
            ...
            try {
                ...
                if (!secondResponse.ok) {
                    ...
                    const errorDetail = secondApiResponseData.error ||
        ...
        const spenderAddress = finalTx.to;
        const approvalResult = await (0, services_1.checkAndApproveTokenAllowance)(wallet, args.tokenInAddress, spenderAddress, BigInt(formattedAmount), // Amount to approve
        true);
        if (!approvalResult.success) {
        ...
        if (approvalResult.userOpHash) {
            console.log(`Main token approval successful for ${args.tokenInAddress} to ${spenderAddress}. UserOpHash: ${approvalResult.userOpHash}`);
        }
        ...
        const transactionToSubmit = {
            to: finalTx.to,
            data: finalTx.data || "0x", // Default to "0x" if undefined
        ...
        try {
            const txResponse = await (0, services_1.sendTransaction)(wallet, transactionToSubmit);
            if (!txResponse.success) {
                console.error("Bridge transaction failed details:", txResponse.error);
                let errorMessage = typeof txResponse.error === "string"
                    ? txResponse.error
...
function formatBigIntToEth(wei) {
...
class SmartBridgeAction {
    constructor() {
        this.name = "smart_bridge";
        this.description = BRIDGE_PROMPT;
        this.argsSchema = exports.SmartBridgeInput;
        this.func = smartBridge;
        this.smartAccountRequired = true; // Requires a smart account for gasless and approvals
    }
}
exports.SmartBridgeAction = SmartBridgeAction;
