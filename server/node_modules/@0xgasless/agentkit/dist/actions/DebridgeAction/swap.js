...
exports.SmartSwapAction = exports.SmartSwapInput = void 0;
exports.smartSwap = smartSwap;
...
`;
exports.SmartSwapInput = zod_1.z
    .object({
    tokenIn: zod_1.z
        .string()
        .optional()
    ...
    tokenInSymbol: zod_1.z
        .string()
        .optional()
        .nullable()
        .describe("The symbol of the input token (e.g., 'ETH', 'USDC')"),
    tokenOutSymbol: zod_1.z
        .string()
        .optional()
        .nullable()
        .describe("The symbol of the output token (e.g., 'ETH', 'USDC')"),
    amount: zod_1.z.string().describe("The amount of input token to swap"),
    slippage: zod_1.z
        .string()
        .optional()
        .nullable()
...
async function smartSwap(wallet, args) {
    try {
        ...
        if (args.tokenInSymbol && !tokenInAddress) {
            const resolved = await (0, services_1.resolveTokenSymbol)(wallet, args.tokenInSymbol);
            if (!resolved) {
        ...
        if (args.tokenOutSymbol && !tokenOutAddress) {
            const resolved = await (0, services_1.resolveTokenSymbol)(wallet, args.tokenOutSymbol);
            if (!resolved) {
        ...
        const formattedAmount = await (0, services_1.formatTokenAmount)(wallet, tokenInAddress, args.amount);
        ...
        try {
            ...
            if (!estimationResponse.ok) {
                if (parsedEstimation.errorMessage?.includes("insufficient liquidity") ||
        ...
        if (!debridgeResponse.ok || transactionData.errorCode == 0) {
            ...
            if (transactionData.errorMessage?.includes("insufficient liquidity") ||
...
3. You have enough balance of the input token`;
            ...
            if (transactionData.errorMessage?.includes("transfer amount exceeds allowance") ||
...
Example: "Swap 0.01 USDT to WETH with approveMax: true"`;
        ...
        if (tokenInAddress !== "0x0000000000000000000000000000000000000000") {
            const spenderAddress = transactionData.tx.to;
            // Check and approve token allowance
            const approvalResult = await (0, services_1.checkAndApproveTokenAllowance)(wallet, tokenInAddress, spenderAddress, BigInt(formattedAmount), args.approveMax ?? false);
        ...
        const txResponse = await (0, services_1.sendTransaction)(wallet, transactionData.tx);
        if (!txResponse.success) {
            if (typeof txResponse.error === "string" &&
...
Example: "Swap 0.01 USDT to WETH with approveMax: true"`;
            ...
            return `Swap failed: ${typeof txResponse.error === "string" ? txResponse.error : JSON.stringify(txResponse.error)}`;
        }
...
class SmartSwapAction {
    constructor() {
        this.name = "smart_swap";
        this.description = SWAP_PROMPT;
        this.argsSchema = exports.SmartSwapInput;
        this.func = smartSwap;
        this.smartAccountRequired = true;
    }
}
exports.SmartSwapAction = SmartSwapAction;
