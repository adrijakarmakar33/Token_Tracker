...
exports.GetTransactionStatusAction = exports.GetTransactionStatusPrompt = exports.GetTransactionStatusSchema = void 0;
exports.getTransactionStatusFunc = getTransactionStatusFunc;
...
exports.GetTransactionStatusSchema = zod_1.z.object({
    userOpHash: zod_1.z.string().regex(/^0x[a-fA-F0-9]{64}$/, "Invalid User Operation Hash format."),
});
// 2. Define the prompt detailing the tool's use
exports.GetTransactionStatusPrompt = `
...
async function getTransactionStatusFunc(wallet, // Wallet instance is passed first
{ userOpHash }) {
    if (!wallet.bundler) {
    ...
    return new Promise(resolve => {
        const intervalId = setInterval(async () => {
            try {
                const receipt = await wallet.bundler.getUserOpReceipt(userOpHash);
                ...
                if (totalDuration >= DEFAULT_MAX_DURATION) {
                    clearInterval(intervalId);
                    resolve(`Transaction is still pending after ${DEFAULT_MAX_DURATION / 1000} seconds. UserOpHash: ${userOpHash}`);
                    return;
                }
            }
            catch (error) {
                ...
                clearInterval(intervalId);
                resolve(`Error polling transaction status for ${userOpHash}: ${error instanceof Error ? error.message : String(error)}`);
...
class GetTransactionStatusAction {
    constructor() {
        this.name = "get_transaction_status";
        this.description = exports.GetTransactionStatusPrompt;
        this.argsSchema = exports.GetTransactionStatusSchema;
        this.func = getTransactionStatusFunc;
        this.smartAccountRequired = true; // Requires wallet to get bundler URL
    }
}
exports.GetTransactionStatusAction = GetTransactionStatusAction;
