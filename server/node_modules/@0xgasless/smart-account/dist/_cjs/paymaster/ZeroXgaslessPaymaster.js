...
exports.ZeroXgaslessPaymaster = void 0;
...
class ZeroXgaslessPaymaster {
    ...
    async getPaymasterFeeQuotesOrData(_userOp, paymasterServiceData) {
        ...
        try {
            ...
            index_js_1.Logger.error("Failed to fetch Fee Quotes or Paymaster data - reason: ", JSON.stringify(error));
            if (!this.paymasterConfig.strictMode &&
                ...
                index_js_1.Logger.warn(`Strict mode is ${this.paymasterConfig.strictMode}. sending paymasterAndData 0x`);
                return {
                    paymasterAndData: "0x",
    ...
    async getPaymasterAndData(_userOp, paymasterServiceData) {
        ...
        try {
            ...
            const response = await (0, index_js_1.sendRequest)(request, "Paymaster");
            if (response?.result) {
                const paymasterAndData = response.result.paymasterAndData;
            ...
            index_js_1.Logger.error("Error in generating paymasterAndData - reason: ", JSON.stringify(error));
            throw error;
        }
        throw new Error("Error in generating paymasterAndData");
    }
    async deployAccount(calldata) {
        try {
            const request = {
                url: `${this.paymasterConfig.paymasterUrl}`,
                method: index_js_1.HttpMethod.Post,
                body: {
                    method: "pm_deployAccount",
                    params: [calldata],
                    id: (0, Helpers_js_1.getTimestampInSeconds)(),
                    jsonrpc: "2.0"
                }
            ...
            index_js_1.Logger.error("Error in deploying account - reason: ", JSON.stringify(error));
            throw error;
        }
        throw new Error("Error in deploying account");
    }
    async getDummyPaymasterAndData(_userOp, _paymasterServiceData) {
        return "0x";
    }
    static async create(config) {
        return new ZeroXgaslessPaymaster(config);
    }
}
exports.ZeroXgaslessPaymaster = ZeroXgaslessPaymaster;
//# sourceMappingURL=ZeroXgaslessPaymaster.js.map