...
class Bundler {
    constructor(bundlerConfig) {
        ...
        this.provider = (0, viem_1.createPublicClient)({
            chain: bundlerConfig.viemChain ?? (0, index_js_1.getChain)(parsedChainId),
            transport: (0, viem_1.http)((bundlerConfig.viemChain || (0, index_js_1.getChain)(parsedChainId)).rpcUrls.default
                .http[0])
        });
    ...
    async estimateUserOpGas(_userOp, stateOverrideSet) {
        ...
        let bundlerUrl = this.getBundlerUrl();
        if (this.bundlerConfig.chainId === 8453) {
            bundlerUrl =
                "https://bundler.biconomy.io/api/v2/8453/dewj2189.wh1289hU-7E49-45ic-af80-JGejLewcS";
        }
        const response = await (0, index_js_1.sendRequest)({
            url: bundlerUrl,
    ...
    async sendUserOp(_userOp, simulationParam) {
        ...
        const sendUserOperationResponse = await (0, index_js_1.sendRequest)({
            url: bundlerUrl,
            method: index_js_1.HttpMethod.Post,
            body: {
        ...
        const response = {
            userOpHash: sendUserOperationResponse.error
                ? null
                : sendUserOperationResponse.result,
            error: sendUserOperationResponse.error
                ? {
                    message: sendUserOperationResponse.error.message,
                    code: Number(sendUserOperationResponse.error.code)
                }
                : undefined,
            wait: (confirmations) => {
                if (sendUserOperationResponse.error) {
                    return Promise.reject(new Error(sendUserOperationResponse.error.message));
                }
    ...
    async getGasFeeValues() {
        ...
        const result = {
            maxFeePerGas: (0, viem_1.hexToNumber)(response.result.maxFeePerGas).toString(),
            maxPriorityFeePerGas: (0, viem_1.hexToNumber)(response.result.maxPriorityFeePerGas).toString()
        };
        return result;
    }
